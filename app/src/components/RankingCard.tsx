...
export function RankingCard({ pnl, rank }: Props) {
  ...
  return (
    <Link href={`/politician/${politician.id}`} className="block">
      <div className="flex items-center gap-2.5 px-5 py-3 bg-white active:bg-gray-50 transition-colors">
        {/* Rank */}
        <div className="w-5 text-center">
          <span className={`text-[12px] font-bold ${rank <= 3 ? "text-toss-blue" : "text-gray-300"}`}>
            {rank}
          </span>
        </div>

        {/* Avatar */}
        <div className="relative w-9 h-9 rounded-full bg-gray-100 flex items-center justify-center overflow-hidden shrink-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1 mt-0.5">
            {pnl.top_movers.slice(0, 3).map((ticker) => (
              <span
                key={ticker}
                className="text-[9px] text-gray-400 bg-gray-50 px-1 py-[1px] rounded"
              >
                {ticker}
              </span>
            ))}
        ...
        <div className="text-right shrink-0">
          <div className={`text-[13px] font-bold ${isPositive ? "text-positive" : "text-negative"}`}>
            {formatPct(pnl.daily_return_pct)}
          </div>
          <div className={`text-[10px] ${isPositive ? "text-positive" : "text-negative"}`}>
            {isPositive ? "+" : ""}
